<?
	...
	$now_query = "select count(*) from tb_userinfo where member_no > '' ".$now_que;
		...
		<table width='99%' bgcolor="#EEEEEE">
			<tr align="center">
				<td align="left">
					...
					<b><input type="radio" name="rsort" value="email_mod_date" <?if($sort == "email_mod_date") echo "checked";?> onClick="check_data();"> 이메일 수정일 </b>
					<b><input type="radio" name="rsort" value="ldate" <?if($sort == "ldate") echo "checked";?> onClick="check_data();"> 최근로그인일 </b>
...
<TR>
	...
	<TH width="10%">연락처</TH>
	<TH width="8%">신청종류</TH>
	<TH width="10%">FO NO</TH>
	<TH width="8%">신청일</TH>
	<TH width="10%">처리상태</TH>
	<TH width="8%">이메일수정일</TH>
<? 	if ($r_status == "A") {?> 
	<TH width="14%">최근로그인</TH>
	<TH width="6%">방문</TH>
<? 	} else if ($r_status == "1") {?> 
	<TH width="14%">최근로그인</TH>
	<TH width="6%">방문</TH>
...
<? 	} else if ($r_status == "9") {?> 
	...
	<TH width="6%">거부자</TH>
<?	}?>
<?	if (($r_status == 1) || ($r_status == 2) || ($r_status == 3)) {?>
	<TH width="4%">선택</TH> 
<?	}?>
</TR>     
<?
/*lkk
$viewlistno=20;
if (($First+20) > $TotalArticle){$viewlistno=$TotalArticle-$First;}
$listno=$First+20;
$query2.=" limit $First,$listno";
echo "query2=".$query2;
*/
#echo $query2;
	...
	if ($TotalArticle) {
		...
		//while ($result2 = mysql_query($query2)) {
			...
			if ($i >= $First) {
				...
				if ($obj->email_mod_date != null) {
					$email_mod_date = date("Y-m-d", strtotime($obj->email_mod_date));
				} else {
					$email_mod_date = "";
				}
...
?>					
<TR align="center">                    
	<TD height="25"><A HREF="javascript:onView('<?echo $obj->member_no?>')"><?echo $obj->name?></A></TD>
	<TD><?echo $obj->reg_jumin1?>-<?echo $obj->reg_jumin2?></TD>
