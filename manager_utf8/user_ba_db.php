<?
	...
	$re_member_no		= str_quote_smart(trim($re_member_no));
	$member_nos			= str_quote_smart(trim($member_nos));
	$numbers				= str_quote_smart(trim($numbers));
	$passwords			= str_quote_smart(trim($passwords));
	...
	for ($i=0 ; count($a_member_nos) > $i ; $i++) {
		...
		$query = "select * from tb_userinfo where member_no = '$a_member_nos[$i]'";
...
<table width='671' height='582' border='0' cellpadding='0' cellspacing='0'>
  <tr>
	...
	echo "<script language=\"javascript\">\n
		alert('입력 되었습니다.');
		document.location = 'user_input_ba.php?member_no=$re_member_no';
		</script>";
	exit;
