<?
	...
	if (!empty($qry_str)) {
		...
		$query = "select count(*) from tb_member_new where member_kind = '$member_kind'  ".$que;
		$query2 = "select * from tb_member_new where member_kind = '$member_kind'  ".$que." order by ".$sort." ".$order ;

	} else {
		$query = "select count(*) from tb_member_new where member_kind = '$member_kind' ";
		$query2 = "select * from tb_member_new where member_kind = '$member_kind'  order by ".$sort." ".$order ;
	}
...
<TR>
<?  if ($member_kind == "C") {?>
	<TD align="left"><B>소비 회원 관리</B></TD>
<?	} else {?>
	<TD align="left"><B>BA 회원 관리</B></TD>
<?  }?>
	<TD align="right" width="600" align="center" bgcolor=silver>
	<SELECT NAME="idxfield">
		<OPTION VALUE="0" <?if($idxfield == "0") echo "selected";?>>BA</OPTION>
...
<tr>
	<td align='center'>
		<table width='99%' bgcolor="#EEEEEE">
			<tr align="center">
				<td align="left">
					<b><input type="radio" name="rsort" value="number" <?if($sort == "number") echo "checked";?> onClick="check_data();"> BA </b>
					<b><input type="radio" name="rsort" value="name" <?if($sort == "name") echo "checked";?> onClick="check_data();"> 회원명 </b>
					<b><input type="radio" name="rsort" value="name" <?if($sort == "appdate") echo "checked";?> onClick="check_data();"> 등록일 </b>
				<td align="right">
