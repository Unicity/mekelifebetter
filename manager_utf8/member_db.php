<?
	...
	function isExist($sid)  { 
		...
		$b = 0;
		$sqlstr = "SELECT COUNT(*) CNT FROM tb_member where reg_no='$sid'"; 
	...
	function getMaxCode()  { 
		...
		$iNewid = 0;
		$sqlstr = "SELECT MAX(member_id) CNT FROM tb_member "; 
	...
	if ($mode == "add") {
		...
		if(!isExist($reg_no) == 1) {
			...
			$query = "insert into tb_member (member_id, reg_no, name, number, phone, zip, addr, temp2, regDate, email, member_kind) values 
		...
		echo "<script language=\"javascript\">\n
			alert('삭제 되었습니다.');
			parent.frames[3].location = 'new_member_list.php?member_kind=$member_kind';
			</script>";
		exit;
