...
<?
	...
	function getMaxCode($scode)  { 
		...
		$iNewid = 0;
		$sqlstr = "SELECT MAX(seq) CNT FROM tb_education_calendar_file where NewsId = '$scode'"; 
	...
	if ($mode == "add") {
		...
		$query = "insert into tb_education_calendar_file ( y,mo,file_name, file_path, wdate, bshow) values 
					  ('$Year1', '$Month1', '$Image_name', '$Image_name', now(),'$bshow')";
		...
		echo "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\">\n
			...
			alert('등록 되었습니다.');
			//parent.frames[3].location = 'edu_cal_file_list.php';
			document.location = 'edu_cal_file_list.php';
			</script>";
		exit;
...
*/
		...
		$Content= VBN_uploadMultiFiles($Content,$v_fileSaveDir,$v_fileSaveBaseURL,$v_fileLimitSize);  //[by 벤처브레인]
		$query = "update tb_education_calendar_file set 
					y = '$Year1',
					mo = '$Month1',
					d = '$Day1',
					h = '$Hour1',
		...
		echo "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\">\n
					<script language=\"javascript\">\n
			alert('수정 되었습니다.');
			//parent.frames[3].location = 'edu_cal_file_list.php?page=$page&idxfield=$idxfield&qry_str=$qry_str';
			document.location = 'edu_cal_file_list.php?page=$page&idxfield=$idxfield&qry_str=$qry_str';
			</script>";
		exit;
		...
		$query = "select * from tb_education_calendar_file 
				    where seq in $id ";
		...
		while($row = mysql_fetch_array($result)) {
			...
			$query_del = "delete from tb_education_calendar_file 
				    where seq = '$SeqNo' ";
		...
		echo "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\">\n
					<script language=\"javascript\">\n
			alert('삭제 되었습니다.');
			//parent.frames[3].location = 'edu_cal_file_list.php?NewsId=$NewsId&page=$page&idxfield=$idxfield&qry_str=$qry_str';
			document.location = 'edu_cal_file_list.php?NewsId=$NewsId&page=$page&idxfield=$idxfield&qry_str=$qry_str';
			</script>";
		exit;

	} else if ($mode == "bshow") {

		$query = "update tb_education_calendar_file set bshow = '$bshow' where seq = '$id' ";
