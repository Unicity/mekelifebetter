...
*/
$query_cnt = "select max(cnt) FROM (SELECT COUNT(LOWER(email)) cnt from tb_useroff where email != '' and  email_send_yn = 'N' and ifnull(del_tf,'N') = 'N' and memo IS NULL GROUP BY LOWER(email) ) AS result";
...
for($j=1; $j<=$totcnt; $j++) {
	$query = "select * from tb_useroff where email != '' and  email_send_yn = 'N' and ifnull(del_tf,'N') = 'N'  and memo IS NULL GROUP BY LOWER(email) order by LOWER(email) ";
	...
	for($i=0; $i<mysql_num_rows($result); $i++) {
		...
		if($row['email'] != ""){
			//if($pre_email != strtolower($row['email'])){
				...
				mysql_query("update tb_useroff set memo ='excel ok' where mno = '$enum' ") or die(mysql_error());
			//}
