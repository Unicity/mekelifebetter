...
$number = $row[number];
//$password = decrypt($key, $iv, $row[password]);  //비밀번호 항목 제거
...
if(in_array('address', $chk)){
	...
	$infodata[mainAddress] = '{"mainAddress":{"city":"'.$city.'","country":"KR","state":"'.$state.'","zip":"'.$row['zip'].'","address1":"'.$address1.'","address2":"'.$address2.'"}}';
...
if(in_array('address', $chk)){
	...
	$data = json_encode($infodata[mainAddress]);
	...
	curl_setopt($ch, CURLOPT_POSTFIELDS, $infodata[mainAddress]);   //주석처리시 회원정보 조회 결과로 리턴됩니다.
	...
	if ($response !== false && ($status == 200 ||$status == 201)) {		
		...
		echo "->Fail<br>"; //.$response."-".$data.'-'.$city.'-'.$state.'-'.$address1;
	}
	flush();
	ob_flush();
}
...
if(in_array('email', $chk)){
	...
	$data = json_encode(array('email' => $row['email']));
...
if(in_array('phone', $chk)){
	...
	$data = json_encode(array('mobilePhone' => $row['hpho1'].$row['hpho2'].$row['hpho3']));
...
if(in_array('tel', $chk)){
	...
	$data = json_encode(array('homePhone' => $row['pho1'].$row['pho2'].$row['pho3']));
...
if(in_array('bank', $chk)){
	...
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL, $member_url.'/depositBankAccount');
	curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
	curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 30); //서버연결에 대한 timeout 
	curl_setopt($ch, CURLOPT_TIMEOUT, 120); //curl 실행에 대한 timeout 
	curl_setopt($ch, CURLOPT_USERPWD, "$username:$password");
	curl_setopt($ch, CURLOPT_HTTPAUTH, CURLAUTH_BASIC);
	curl_setopt($ch, CURLOPT_POSTFIELDS, $postdata);   //주석처리시 회원정보 조회 결과로 리턴됩니다.
	$response = curl_exec($ch);			 
	$status = curl_getinfo($ch, CURLINFO_HTTP_CODE);
	if ($response !== false && ($status == 200 ||$status == 201)) {		
		$data = json_decode($response);

		if($data->bin == $search_bank_code && $data->accountNumber == $account) echo "->OK<br>";
		else echo "->Fail<br>".print_r($response);
	}else{
		echo "->Fail<br>".$response."-".$data;
	}
...
if(in_array('agree', $chk)){	
	...
	foreach($rightsArray as $key=>$val) {
		...
		if($status == "201" || $status == "204"){  //delete인경우 204
			...
			echo "->Fail".$status."-".print_r($result)."<br>";		
		}
		flush();
		ob_flush();
