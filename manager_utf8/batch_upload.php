...
function js_send(n) {
	...
	if(n == ""){
		...
		if(confirm("API일괄발송 처리를 하시겠습니까?")){
			location.href =  "batch_process.php?grp="+n;
		}
	}
}
...
<?php 
//그룹 count
$result = mysql_query("select max(grp) from tb_batch_agree") or die(mysql_error());	
	...
	if(strtolower(end(explode(".", $_FILES['upfile']['name']))) != "txt"){
		...
		while (($line = fgets($fp)) !== false) {
			...
			if($data[0] != ""){
				...
				$query = "select * from (
						select 'user' as tbl, number, name, password, agree_01, agree_02, agree_03, sel_agree01, sel_agree02, sel_agree03, sel_agree04, sel_agree05, regdate  from tb_userinfo where number = '".$data[0]."'
						union all
						select 'dup' as tbl, number, name, password, agree_01, agree_02, agree_03, sel_agree01, sel_agree02, sel_agree03, sel_agree04, sel_agree05, regdate  from tb_userinfo_dup where number = '".$data[0]."'
				...
				if($row['number'] == ""){
					...
					$row['korname'] = urlencode($row['name']);
					$insql = "insert into tb_batch_agree set
								grp = '".$group."',						
								number = '".$row['number']."',
								tbl = '".$row['tbl']."',
								name = '".$row['name']."',
								password = '".$row['password']."',
								agree_01 = '".$row['agree_01']."',
								agree_02 = '".$row['agree_02']."',
								agree_03 = '".$row['agree_03']."',
								sel_agree01 = '".$row['sel_agree01']."',
								sel_agree02 = '".$row['sel_agree02']."',
								sel_agree03 = '".$row['sel_agree03']."',
								sel_agree04 = '".$row['sel_agree04']."',
								sel_agree05 = '".$row['sel_agree05']."',
								regDate = '".$row['regdate']."', 
								createdDate = now()";	
					mysql_query($insql) or die(mysql_error());	

					echo ($i+1).": ".$data[0].", ".$row['name'].", ".$row['regdate'].", ".$row['tbl']."<br>"; //.$row['password']."<br>";
					$success++;
				}
			}
			$i++;
