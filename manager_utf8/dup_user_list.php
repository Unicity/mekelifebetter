<?
	...
	logging($s_adm_id,'open dup user list (dup_user_list.php)');
	...
	*/
	...
	if (!empty($qry_str)) {
		...
		logging($s_adm_id,'search dup user '.$que);
	...
	$query = "select count(*) from tb_userinfo_dup where 1 = 1 ".$que;
	$result = mysql_query($query,$connect);
	$row = mysql_fetch_array($result);
	$TotalArticle = $row[0];
	logging($s_adm_id,'search dup user count '.$TotalArticle);

	$query2 = "select * from tb_userinfo_dup where 1 = 1 ".$que." order by ".$con_sort." ".$order." limit ". $offset.", ".$nPageSize; ;
	$result2 = mysql_query($query2);

	$now_que = " and date_format(date_sub(regdate, interval 0 day),'%Y-%m-%d') = date_format(now(),'%Y-%m-%d') ";	
	$now_query = "select count(*) from tb_userinfo_dup where 1 = 1 ".$now_que;
...
function check_data(){
	...
	document.frmSearch.action="dup_user_list.php";
...
function onView(id) {
	...
	document.frmSearch.action= "dup_user_view.php";
	document.frmSearch.submit();
}

function goIn() {
	document.frmSearch.action= "dup_user_input.php";
	document.frmSearch.submit();
}
...
function goDel() {
	...
	if ( bDelOK ==true ) {
		...
		document.frmSearch.action = "dup_user_db.php";
		document.frmSearch.submit();
	}
	else {
		return;
...
function goInputAllBA(){
	...
	if ( bDelOK ==true ) {
		document.frmSearch.action = "dup_user_ba_db2.php";			
		document.frmSearch.submit();
	} else {
		return;
	}
...
function goConfirm() {
	...
	var url = "dup_user_confirm.php?member_no="+document.frmSearch.member_no.value;
...
function goPrint() {
	...
	var url = "dup_user_print.php?member_no="+document.frmSearch.member_no.value;
...
function goPrint2() {
	...
	var url = "dup_user_print_test.php?member_no="+document.frmSearch.member_no.value;
...
function goInputBA() {
	...
	var url = "dup_user_input_ba.php?member_no="+document.frmSearch.member_no.value;
...
function goEmail(member_no) {
	var url = "dup_user_email_ba.php?member_no="+member_no+"&munja=Y";
...
function goReSend(member_no) {
	...
	if (bDelOK==true) {
		<?php //if($_SERVER['HTTP_X_FORWARDED_FOR'] == "121.190.224.191") {?>
			...
			document.frm_m.member_no.value = member_no;
			document.frm_m.action = "dup_user_resend_api.php";
			document.frm_m.submit();
			*/
		<?php //}else{ ?>
			var url = "dup_user_reg_ba.php?member_no="+member_no+"&munja=Y";
...
<TR>
	<TD align="left"><B>온라인갱신 회원 관리</B></TD>
		...
		<table width='99%' bgcolor="#EEEEEE">
			<tr align="center">
				<td align="left">
					...
					<input type="radio" name="r_status" value="F" <? if ($r_status == "F") echo "checked" ?>  onClick="check_data();"> 통신장애&nbsp;&nbsp;
				</td>
			</tr>
		</table>
	</td>
...
<table width="100%" border="0">
	<tr>
		<td width="100%" align="right">
			...
			<font color="red">이름이 빨간색으로 표시 되었을 경우 통신장애에 의해 회원가입 여부가 확인 되지 않은 경우임</font>&nbsp;&nbsp;<br>
			인증방식안내 : 모 (모바일)&nbsp;&nbsp;<!-- 신 (신용카드)&nbsp;&nbsp; -->공 (공인인증서)&nbsp;&nbsp;아 (아이핀)&nbsp;&nbsp;카 (카카오)&nbsp;&nbsp;<!-- [시간]&nbsp;&nbsp; -->
		</td>
	</tr>
</table>
...
?>
<TR align="center">
	<TD height="25"><A HREF="javascript:onView('<?echo $obj->member_no?>')"><?echo $str_name?></A></TD>
...
<TR>
	<TD align="left">
	    조회 회원 수 : <?echo $TotalArticle?> 명, 오늘 접수된 회원 : <?//echo $TotalArticle3?><?echo $now_TotalArticle;?>명 
	</TD>
	<TD align="right">
<?
...
<form name='frm_m' id='frm_m' method="POST">
<input type="hidden" name="member_no" value="">
</form>
