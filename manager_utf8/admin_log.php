...
function onSearch(){
	...
	document.frmSearch.action="admin_log.php";
...
function js_excel(){
	...
	*/
	...
	//var diff = dateDiff(f.from_date.value, f.to_date.value);	

	//if(diff > 31){
	//	alert("다운로드 기간은 최대 31일 이내로 설정하여 주세요");
	//	f.to_date.focus();
	//}else{
		if(confirm("엑셀다운로드를 하시겠습니까?\n종료일 1일 로그데이터만 다운로드 됩니다\n다소 시간이 소요되오니 확인 후 잠시 기다려 주세요")){
			location.href = "admin_log_excel.php?from_date=" + f.from_date.value + "&to_date=" + f.to_date.value + "&idxfield=" + f.idxfield.value + "&qry_str=" + f.qry_str.value;
		}
	//}
}
...
function dateDiff(_date1, _date2) {
   ...
   var diffDate_1 = new Date(Number(_date1.substr(0,4)), Number(_date1.substr(4,2)), Number(_date1.substr(6,2)));
   var diffDate_2 = new Date(Number(_date2.substr(0,4)), Number(_date2.substr(4,2)), Number(_date2.substr(6,2)));
			...
			<tr>
				...
				<td>
					...
					<?php //if(getRealClientIp() == "121.190.224.85"){ ?>
						...
						&nbsp;&nbsp; <span style="color:#336699">데이터 용량이 큰 관계료 1일 로그만 다운로드 됩니다.(종료일 기준)</span>
					<?php //} ?>
				</td>
			</tr>
			<tr>
