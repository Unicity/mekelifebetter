<? 
			...
			if($i < $total )	{ 
				...
				if ($ii_cnt == 0) {
					...
					if ($i == ($total-1)) {
						...
						print("<td><a href='#'><img src='../manager/images/close.gif' id=$row[distributor_id]f border=0 ></a></td>\n<td>$local_name <font color=red>$row[distributor_id]</font> [$row[base_personal_volume]] [$row[base_team_volume],$row[test_tv]]</td>\n");
	...
	function sum_pv ($distributor_id, $pv, $tv)  { 	
		...
		if ($total > 0) {
			...
			$sqlstr = "SELECT base_personal_volume, base_team_volume FROM tb_distrib_perfs WHERE distributor_id = '$next_distributor_id'"; 
					
			$result2 = mysql_query($sqlstr);
			$list2 = mysql_fetch_array($result2);

			$next_tv = $list2[base_team_volume];
			$next_pv = $list2[base_personal_volume];
			...
			if ($next_pv >= 100) {
				...
				$sqlstr1 = "update tb_distrib_perfs set test_tv = test_tv + '$sum_pv' where distributor_id = '$next_distributor_id'";
				mysql_query($sqlstr1) or die("Query Error");
				...
				$sqlstr1 = "update tb_distrib_perfs set test_tv = '$next_pv' where distributor_id = '$next_distributor_id'";
				mysql_query($sqlstr1) or die("Query Error");
			...
			sum_pv ($next_distributor_id, $pv, $tv);				
		} 
	}
...
<div id='treeViewer'>
<?
	$sqlstr1 = "update tb_distrib_perfs set test_tv = '0' ";
	mysql_query($sqlstr1) or die("Query Error");
