...
if($member_no == '' || $number == ''){
	...
	//회원번호가 없는 경우 -  신규가입 API 재전송 처리
	$result = mysql_query("select password from tb_userinfo_dup where member_no = '".$member_no."'") or die(mysql_error());		
	$row = mysql_fetch_array($result);
	
	echo $row['password']."<br>";
	echo $password = decrypt($key, $iv, $row[password]);
