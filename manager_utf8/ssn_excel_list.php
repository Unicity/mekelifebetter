...
ini_set('max_execution_time', 60);
	...
	*/
	...
	if (!empty($qry_str)) {
		...
		////eycho 예외처리
		//if($_SESSION["s_adm_id"] == "eycho") $query2 = "select * from tb_distSSN where 1 ".$que." order by ".$con_sort." ".$order ;
		//else 
		$query2 = "select * from tb_distSSN where id in (".$idVal.") ".$que." order by ".$con_sort." ".$order ;
	} else {
		
		//eycho 예외처리
		//if($_SESSION["s_adm_id"] == "eycho") $query2 = "select * from tb_distSSN where 1  order by ".$con_sort." ".$order ;
		//else 
		$query2 = "select * from tb_distSSN where id in (".$idVal.")  order by ".$con_sort." ".$order ;
	}
