...
//	mysql_query($query) or die("Query Error".$query);
	...
	$new_cfile = "c_member.txt";
	$new_dfile = "d_member.txt";
...
*/
/*
	$query = "select b.zip, b.addr, b.number from tb_userinfo a, tb_member b where a.number = b.number ";
	...
	while($row = mysql_fetch_array($result)) {
		...
		$addr = eregi_replace("'", "''", $addr);
	   	...
	   	$query = "update tb_userinfo set zip = '$zip', addr = '$addr' where number = '$number'";
		mysql_query($query) or die("Query Error".$query);
