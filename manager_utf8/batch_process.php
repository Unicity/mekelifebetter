...
$result = mysql_query("select * from tb_batch_agree where grp = '".$_GET['grp']."'") or die(mysql_error());	
$num = mysql_num_rows($result);

if($num < 1){
	echo "<script>
	alert('발송대상이 없습니다');
	location.replace('batch_agree.php');
	</script>";
	exit;
}
while($row=mysql_fetch_array($result)) {
	...
	$rs['agree'] = 	$row['agree_01'].",".$row['agree_02'].",".$row['agree_03'].",".$row['sel_agree01'].",".$row['sel_agree02'].",".$row['sel_agree03'].",".$row['sel_agree04'].",".$row['sel_agree05'];
	...
	for(i = 0; i < total; i++){
		...
		if(data[i].pwd == ""){
			...
			$.ajax({
				type: 'post',
				url: 'batch_run.php',
				data: {'name': data[i].name, 'number': data[i].number, 'pwd': data[i].pwd,  'agree': data[i].agree, 'tbl': data[i].tbl},
				//async: false,
...
<div id="btnGroup" style="width:94%;; text-align:center; padding:10px 0px 30px; display:none"><a href="batch_agree.php" class="btn">확인</a></div>
