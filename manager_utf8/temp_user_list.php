<?
	...
	logging($s_adm_id,'open temp user list (temp_user_list.php)');
	...
	if (!empty($qry_str)) {
		...
		logging($s_adm_id,'search temp user '.$que);
	...
	$query = "select count(*) from tb_userinfo_temp where DEL_TF = 'N' ".$que;
	$result = mysql_query($query,$connect);
	$row = mysql_fetch_array($result);
	$TotalArticle = $row[0];
	logging($s_adm_id,'search temp user count '.$TotalArticle);

	$query2 = "select * from tb_userinfo_temp where DEL_TF = 'N' ".$que." order by ".$con_sort." ".$order." limit ". $offset.", ".$nPageSize; ;
	$result2 = mysql_query($query2);

	$now_que = " and date_format(date_sub(regdate, interval 0 day),'%Y-%m-%d') = date_format(now(),'%Y-%m-%d') ";	
	$now_query = "select count(*) from tb_userinfo_temp where DEL_TF = 'N' ".$now_que;
...
function check_data(){
	...
	document.frmSearch.action="temp_user_list.php";
...
function onView(id) {
	...
	document.frmSearch.action= "temp_user_view.php";
	document.frmSearch.submit();
}

function goIn() {
	document.frmSearch.action= "temp_user_input.php";
	document.frmSearch.submit();
}
...
function goDel() {
	...
	if ( bDelOK ==true ) {
		...
		document.frmSearch.action = "temp_user_db.php";
		document.frmSearch.submit();
	}
	else {
		return;
...
function goInputAllBA(){
	...
	if ( bDelOK ==true ) {
		document.frmSearch.action = "temp_user_ba_db2.php";			
		document.frmSearch.submit();
	} else {
		return;
	}
...
function goConfirm() {
	...
	var url = "temp_user_confirm.php?member_no="+document.frmSearch.member_no.value;
...
function goPrint() {
	...
	var url = "temp_user_print.php?member_no="+document.frmSearch.member_no.value;
...
function goPrint2() {
	...
	var url = "temp_user_print_test.php?member_no="+document.frmSearch.member_no.value;
...
function goInputBA() {
	...
	var url = "temp_user_input_ba.php?member_no="+document.frmSearch.member_no.value;
...
function goEmail(member_no) {
	var url = "temp_user_email_ba.php?member_no="+member_no+"&munja=Y";
...
function goReSend(member_no) {
	...
	if (bDelOK==true) {
		var url = "temp_user_reg_ba.php?member_no="+member_no+"&munja=Y";
...
<TR>
	<TD align="left"><B>임시저장 회원 관리</B></TD>
		...
		<table width='99%' bgcolor="#EEEEEE">
			<tr align="center">
				<td align="left">
					...
					<b><input type="radio" name="rsort" value="number" <?if($con_sort == "number") echo "checked";?> onClick="check_data();"> FO </b>
...
?>
	<TD><?echo decrypt($key, $iv, $obj->JU_NO01)?></TD>
