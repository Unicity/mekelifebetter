...
if($_SESSION["S_FLAG"] =='kakao'){
	...
	if($k_phone_number=='01020090796' ||$k_phone_number=='01041765786' || $k_phone_number=='01093205592' ){
	echo $k_birthday;
	echo $k_gender;
	echo $k_phone_number;
	}
...
<div class="cont_wrap">
	...
	<form name="applicationForm" id="frm" method="post" action="#">
		...
		<div class="cont_from">
			...
			<table>
				...
				<tr>
					<td><div><label for="koreanName" class="point wid_4">한글성명</label> <input id="koreanName" type="text" name="koreanName" value="<?=$_SESSION["S_NM"]?>" title="한글성명" maxlength="20"  <?=($_SESSION["S_FLAG"] !='kakao')?"readonly":"";?>   maxlength="6"> </div></td>
				...
				<tr>
					<td class="all" colspan="4"><div><label for="id6" class="point wid_7">온라인 비밀번호</label> <input id="password" type="password" name="password" value="" placeholder="10자이상 숫자/영문 조합" autocomplete="new-password" maxlength="20"> </div></td>
				</tr>
	...
	<div class="btn_box">
		<button type="button" name="button" class="btn btn_color_1 submitRegisterBtn" onclick="submitForm()">가입하기</button>
...
<?php if(getRealIp() == "116.34.66.3"){ ?>
<iframe name="ifrHidden" name="ifrHidden" src="about:blank" style="display:block; width:100%; height:500px; border:1px solid #000"></iframe>
<?php }else{ ?>
