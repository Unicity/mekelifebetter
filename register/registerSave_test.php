...
echo "sponsorNo:".$sponsorNo;
echo "sponsorName:".$sponsorName;
//로그v2저장
$qlog = "insert into tb_log_v2 (tmpId, gubun, name, jumin1, sendData, recieveData, flag, device, logdate) values ( '".$_SESSION['ssid']."', '회원가입처리시작', '$name', '".$dob."', '', '', 'Y', '$user_device', now())";	
@mysql_query($qlog);


//가입정보 temp 저장
$temp_no = insertMemeber('tb_userinfo_temp', $encPassword, $name, $englishName, $encDob, $encGender, $distType, $phoneNo1, $phoneNo2, $phoneNo3, $mobileNo1, $mobileNo2, $mobileNo3, $zip, $fullAddress, $detailAddress, $email, $encAccountNo, $bankName, $sponsorNo, $sponsorName, $type, $birthYear, $birthMonth, $birthDate, $gender, $bankcode, $agree_01, $agree_02, $agree_03, $agree_04, $agree_05, $selchk1, $selchk2, $selchk3, $selchk4, $selchk5, $selchk6, $user_device,$k_id);
...
if(isExist($name, $birthYear, $birthMonth, $birthDate) == 0){  //신규회원
	...
	if(($member_no != "") && !is_null($member_no)){
		...
		if ($member_no != "" && $newMemberId  !="" && !empty($newMemberId)) {
		...
		if($member_no != ''){
			...
			//$api_fail = 'Y', $reg_status = 'F'; 통신장애
			//mysql_query("update tb_userinfo_dup set api_fail = 'Y', reg_status = 'F' where member_no = '".$member_no."'");
		}
