...
/*	$query ="
	...
	$query ="SELECT 	
			...
			FROM (select User.id, contestDate.date,User.isCleanser  FROM User,contestDate 
				...
				*/
	$query = "SELECT * FROM  (SELECT record.ProductID, Product.ProductName, record.Step, record.amount, record.CreateDate  FROM (SELECT ProductID, Step, SUM(Amount) AS amount, Date_format(CreateDate, '%Y-%c-%e') AS CreateDate FROM 5step.StepRecord WHERE ProgramID = '$programID' AND Date_format(CreateDate, '%Y-%c-%e') = '$selectedDate' AND member_no = '$id' GROUP BY ProductID, Step, Date_format(CreateDate, '%Y-%c-%e') ) AS record JOIN Product ON record.ProductID = Product.ProductID AND record.Step = Product.Step ".
		"UNION 
 		SELECT `type` as ProductID,
                case `type` when 'A' then '총콜레스테롤'
                            when 'B' then 'HDL'
                            when 'C' then 'LDL'
                    		when 'D' then '중성지방'
                    		when 'E' then '체중'
                    		when 'F' then '체지방률'
                    		else 'none' END AS 'ProductName',
                6 as Step,
        		checkValue as Amount,
        		Date_format(createDate, '%Y-%c-%e') AS CreateDate
		FROM healthCheck 
		WHERE id = '$id' AND Date_format(createDate, '%Y-%c-%e') = '$selectedDate') AS Record ORDER BY Record.Step, Record.ProductID";
