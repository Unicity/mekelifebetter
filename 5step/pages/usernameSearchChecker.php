           ...
           $('#registerform').data('bootstrapValidator').resetForm();
            ...
            $.post($form.attr('action'), $form.serialize(), function(result) {
                ...
                if (result !== 'fail' && result !== '') {
                	...
                	$('#searchResult').html("<font style='color:green; font-size:18px; text-align:center;'>"+ids.length+"개의 아이디가 검색 되었습니다.<br>"+ids+"</font>");

                } else {
                    console.log(result);
                	$('#searchResult').html("<font style='color:red;'>입력된 데이터와 일치하는 사용자가 없습니다.</font>");
                	$("#loader").removeClass("loader");
    				$("#content").css("opacity", "1");
                }
            });
...
<div class="container" id="content">
	<div style="text-align: center;margin: 20px 0; padding 30px; 0;">
		 ...
		 <div style="padding: 15px 0; font-size: 15px; color:#aaaaaa;">아이디 검색을 위해 정보를 입력하세요.</div>
	</div>
	 <form name="registerform" id="registerform" class="form-horizontal" action="../inc/usernameSearchProcess.php" novalidate>
        ...
        <div id='searchResult' style="width:98%; text-align: center;"></div>
		<div class="col-sm-12" style="text-align: center; color: #919191; margin-top: 10px;"> 
			<button type="submit" style="background-color: white; color: black; border: 2px solid #4CAF50;padding: 10px 80px; text-align: center; text-decoration: none; display: inline-block; border-radius: 10px; font-size: 16px; margin: 4px 2px; -webkit-transition-duration: 0.4s; transition-duration: 0.4s; cursor: pointer;">아이디찾기</button>
		</div>
	</form>
